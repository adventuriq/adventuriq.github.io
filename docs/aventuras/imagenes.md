---
sidebar_position: 5
title: Imágenes
---

# Imágenes de la Aventura

## Imágenes del listado

Las imágenes que se muestran en el **listado de Aventuras** son la lista visual de Aventuras que un jugador tiene cargadas en su perfil de usuario.

- Haciendo **TAP** encima de la imagen, se obtiene una visibilidad del 100%.
- Los formatos gráficos permitidos son: **JPEG, PNG y GIF**.
- Los tamaños de las imágenes son recomendados.

:::caution
Dependiendo del dispositivo donde se juegue, las imágenes pueden aparecer cortadas. Siempre valida en la Web App cómo quedan los contenidos.
:::

## Dónde aparecen las imágenes

- En el **listado de Aventuras** del jugador.
- En la **cabecera de la Aventura**, junto con las fechas de inicio y final automáticos.
